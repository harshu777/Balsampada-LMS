...
export class EmailService {
  ...
  private readonly appName = 'Balsampada';

  constructor(private configService: ConfigService) {
    // For Gmail SMTP (FREE)
    this.fromEmail = this.configService.get<string>('MAIL_FROM') || 'noreply@tuitionlms.com';
    ...
    this.transporter = nodemailer.createTransport({
      ...
      auth: {
        ...
        pass: this.configService.get<string>('MAIL_PASSWORD'), // App-specific password for Gmail
      },
    });
  ...
  async sendVerificationEmail(email: string, name: string, token: string): Promise<boolean> {
    ...
    const logoUrl = `${this.configService.get<string>('FRONTEND_URL')}/logo.png`;
  ...
  async sendApprovalEmail(email: string, name: string, status: 'APPROVED' | 'REJECTED', reason?: string): Promise<boolean> {
  ...
  async sendClassReminderEmail(
    ...
    const logoUrl = `${this.configService.get<string>('FRONTEND_URL')}/logo.png`;
    const dashboardUrl = `${this.configService.get<string>('FRONTEND_URL')}/student/dashboard`;
  ...
  async sendWelcomeBackEmail(email: string, name: string, role: string): Promise<boolean> {
    const dashboardUrl = `${this.configService.get<string>('FRONTEND_URL')}/${role.toLowerCase()}/dashboard`;
    const logoUrl = `${this.configService.get<string>('FRONTEND_URL')}/logo.png`;
